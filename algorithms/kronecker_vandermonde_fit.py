...
def eigenvalue_distance(
    reference: NDArray[np.complexfloating], estimate: NDArray[np.complexfloating]
) -> NDArray[np.floating]:
    """
...
class BlockVandermondeFit(ABC):
    ...
    def compute_features(
        ...
        **kwargs,
        ...
        return_raw : bool
            Whether to return raw error metrics in addition to scores.
        **kwargs
        ...
        if eigenvalues.ndim != 1 or eigenvalues.shape[0] != modes.shape[1]:
            raise ValueError(f"eigenvalues must be shape (M,) matching number of modes")
        ...
        if not return_raw:
            features = {k: v for k, v in features.items() if not k.endswith("_raw")}
    ...
    def _reshape_modes_to_matrices(
        ...
        return (
            modes.T.reshape(num_modes, self.num_delays, self.spatial_dim)
            .transpose(0, 2, 1)
    ...
    @staticmethod
    def _plot_mode_matrices(mode_matrices: NDArray[np.complexfloating]):
        ...
        **fit_dmd_kwargs,
        ...
        nested_eigenvalues = np.array(
            [
                (
                    fit.eigs[0]
                    if hasattr(fit, "eigs") and len(fit.eigs) >= 1
