...
class STC:
    ...
    def __init__(
        ...
        self.omega_s = 2 * np.pi / dt  # Sampling frequency
    ...
    def compute_features(
        ...
        oscillation_rates = np.abs(np.log(eigenvalues)) / self.dt
        ...
        if plot:
            ...
            scatter_scores_1d(scores, "STC", title="STC Consistency Scores", show_id=True)
    ...
    def _compute_max_valid_lags(
        ...
        max_lags = np.minimum(
            self.num_delays - 1,
            np.floor(0.5 * self.omega_s / oscillation_rates)
        )
