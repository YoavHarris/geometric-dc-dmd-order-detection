...
class ExperimentRunner:
    ...
    def submit(
        ...
        for job_id in final_ids:
            job_config_path = job_configs_dir / f"job{job_id:04d}.yaml"
            ...
            script_path = output_dir / f"pbs_scripts" / f"job_{job_id:04d}.sh"
            ...
            if stdout:
                ...
                failed_count += 1
                print(f"  ✗ Job {job_id:04d} submission failed: {stderr}")
    ...
    def status(self, config: str) -> None:
        ...
        print(f"Pending:          {total_jobs - len(succeeded) - len(running) - len(failed)}")
