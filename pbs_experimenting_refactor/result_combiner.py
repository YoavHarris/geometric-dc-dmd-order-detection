...
def load_job_result(output_dir: Path, job_id: int) -> pd.DataFrame:
    ...
    """
    csv_path = output_dir / "job_outputs" / f"job{job_id:04d}" / "results.csv"
...
def validate_and_deduplicate(df: pd.DataFrame) -> tuple[pd.DataFrame, list]:
    ...
    valid_fields = [f for f in UNIQUE_FIELDS if f in df.columns and not df[f].isna().all()]
...
def combine_results(
    ...
    if incremental and combined_path.exists():
        ...
        existing_ids = set(existing["job_id"].unique()) if "job_id" in existing.columns else set()
        ...
        new_dfs = [load_job_result(base_dir, jid) for jid in tqdm(new_ids, desc="Loading new results")]
        full_df = pd.concat([existing] + new_dfs, ignore_index=True)
    else:
        # Full combine from scratch
        print(f"Combining {len(job_ids)} jobs")
        job_dfs = [load_job_result(base_dir, jid) for jid in tqdm(job_ids, desc="Loading results")]
