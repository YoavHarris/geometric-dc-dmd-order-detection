...
def _skip_run_with_empty_csv(
    ...
    summary_cols = [
        # parameters
        "snr_db", "freq_sep", "eig_mag", "eig_mag_spread", "rho_mode",
        "delays_over_timesteps", "num_delays", "temporal_dim", "spatial_dim",
        "num_modes", "top_amplitude", "max_rank", "artificial_damping",
        "n_iter", "dt", "noise_mode", "base_random_seed", "random_seed",
        "method",
        # metrics
...
def cluster_scores(
    ...
    clustering = ModeClustering(
        ...
        decider=clustering_config.get("decider", "mean"),
        normalization_mode=clustering_config.get("normalization", "min_max"),
    )
...
class MethodEvaluator:
    ...
    def evaluate(
        ...
        if self.num_delays > 1:
            delay_methods = [m for m in self.enabled_methods if m in DELAY_EMBEDDING_METHODS]
            if delay_methods:
    ...
    def _evaluate_delay_methods(
        ...
        # STC
        if "STC" in methods:
            stc = STC(num_delays=self.num_delays, dt=self.dt)
    ...
    def to_dataframe(
        ...
        for method in methods:
            ...
            summary.update(
                ...
                bias_skewness=(
                    scipy.stats.skew(diffs) if diffs.size and not np.all(diffs == diffs[0])
                    else np.nan
                ),
...
def compute_dmd_and_ground_truth(
    ...
    proj_dmd = fit_dmd(signal, svd_rank=max_rank, mode="projected", num_delays=num_delays)
...
def run_experiment(job_config: Dict, plot: bool = False) -> None:
    ...
    print(
        summary_df[[
            "method", "abs_diff_mean", "order_hit_prob",
            "bias_frac_over", "bias_frac_under",
