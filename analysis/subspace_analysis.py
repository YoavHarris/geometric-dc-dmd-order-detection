...
def make_vandermonde_matrix(
    eigenvalues: NDArray[np.complex128], num_delays: int
...
def orthonormal_basis(
    matrix: NDArray[np.complex128], rank: int | None = None
) -> NDArray[np.complex128]:
    """
...
def compute_estimated_basis(
    embedded_data: NDArray[np.complex128], num_modes: int
) -> NDArray[np.complex128]:
    """
...
def compute_subspace_principal_angles(
    ...
    if total_dim % spatial_dim != 0:
        raise ValueError(
            f"embedded_data rows ({total_dim}) must be multiple of spatial_dim ({spatial_dim})."
        )
    num_delays = total_dim // spatial_dim

    if true_modes.shape[1] != num_modes or true_eigenvalues.shape[0] != num_modes:
        raise ValueError(
            f"true_modes must be ({spatial_dim}, {num_modes}), "
