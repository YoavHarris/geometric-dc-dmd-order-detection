...
class SingleScanPlotter:
    ...
    def plot(
        ...
        if working_point and x_param in working_point:
            ...
            ax.axvline(wp_value, color='black', linestyle='--', 
        ...
        ax.set_xlabel(self._format_label(x_param),
                     fontsize=self.config.get('labels', {}).get('fontsize', 10))
        ax.grid(True, alpha=0.3)
        
        if title:
            ax.set_title(title, 
                        fontsize=self.config.get('labels', {}).get('title_fontsize', 14))
...
class PanelComposer:
    ...
    def compose(
        ...
        if layout == 'horizontal':
            ...
            fig, axes = plt.subplots(1, n_panels, figsize=figsize, dpi=dpi, squeeze=False)
            axes = axes[0]
        else:  # vertical
            figsize = (panel_width, panel_height * n_panels)
            fig, axes = plt.subplots(n_panels, 1, figsize=figsize, dpi=dpi, squeeze=False)
            axes = axes[:, 0]
        ...
        for i, panel_spec in enumerate(panels):
            ...
            if show_legend and layout == 'horizontal':
                legend_cfg = self.config.get('legend', {})
                axes[i].legend(loc='center left', bbox_to_anchor=(1.02, 0.5),
