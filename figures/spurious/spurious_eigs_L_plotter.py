...
def validate_and_load_data(config: dict[str, Any], project_root: Path) -> pd.DataFrame:
    ...
    print(
        f"Unique L values ({len(df_filtered['L'].unique())}): {sorted(df_filtered['L'].unique())}"
    )
    print()
...
def _resolve_L_values(config: dict[str, Any], available_L: list[int]) -> list[int]:
    ...
    if len(valid) != len(requested):
        print(
            f"Warning: Requested L values missing from data: {set(requested) - available_set}"
        )
...
def plot_cdf_per_L(
    ...
    if aggregation_method not in ["pooled", "trial_averaged"]:
        raise ValueError(
            f"aggregation_method must be 'pooled' or 'trial_averaged', got: {aggregation_method}"
        )
    ...
    for L, color in zip(L_values, colors):
        if aggregation_method == "pooled":
            ...
            if "trial_id" not in df.columns:
                raise ValueError(
                    "aggregation_method='trial_averaged' requires 'trial_id' column"
                )
            ...
            for trial_id in trial_ids:
                trial_vals = df_L[df_L["trial_id"] == trial_id][
                    "eigenvalue_magnitude"
                ].to_numpy()
                trial_vals_sorted = np.sort(trial_vals)
                # Interpolate CDF at common evaluation points
                cdf_interp = np.searchsorted(
                    trial_vals_sorted, x_eval, side="right"
                ) / len(trial_vals_sorted)
...
def plot_rho_min_vs_L(
    ...
    rho_min = (
        df_plot.groupby(["L", "trial_id"])["eigenvalue_magnitude"].min().reset_index()
    )
    ...
    ax.plot(
        L_values, medians, marker="o", linestyle="-", label=r"Median($\rho_{\min}$)"
    )
    ax.plot(
        L_values,
        means,
    ...
    p_high_str = (
        f"{p_high:.1f}".rstrip("0").rstrip(".") if p_high % 1 else f"{int(p_high)}"
    )
    ax.fill_between(
        L_values, lowers, uppers, alpha=0.3, label=f"{p_low_str}–{p_high_str}% band"
    )
...
def plot_combined_cdf_comparison(
    df: pd.DataFrame, config: dict[str, Any], output_path: Path
) -> None:
    """
    ...
    if "setting" not in df.columns or "trial_id" not in df.columns:
        print(
            "Warning: Skipping combined CDF comparison - requires 'setting' and 'trial_id' columns"
        )
        return
    ...
    rho_min = (
        df.groupby(["L", "trial_id", "setting"])["eigenvalue_magnitude"]
        .min()
        .reset_index()
    )
    rho_min.rename(columns={"eigenvalue_magnitude": "rho_min"}, inplace=True)
    rho_min_mix = rho_min[rho_min["setting"] == "mixture_spurious"][
        "rho_min"
    ].to_numpy()
    ...
    fig.legend(
        handles=[line_mix, line_noise, ref_line],
...
def plot_cdf_and_rho_min_combined(
    df: pd.DataFrame, config: dict[str, Any], output_path: Path
) -> None:
    """
    ...
    if "trial_id" not in df.columns:
        print(
            "Warning: Skipping combined CDF+rho_min plot - requires 'trial_id' column"
        )
        return
    ...
    for L, color in zip(L_values, colors):
        ...
        ax_left.step(
            xs_plot, ys_plot, where="post", label=f"L={L}", color=color, alpha=0.8
        )
    ...
    rho_min = (
        df_mix.groupby(["L", "trial_id"])["eigenvalue_magnitude"].min().reset_index()
    )
    rho_min.rename(columns={"eigenvalue_magnitude": "rho_min"}, inplace=True)
...
def plot_mu_nu_vs_L(
    ...
    """
    if (
        "mu_L" not in df.columns
        or "nu_L" not in df.columns
        or "trial_id" not in df.columns
    ):
        print(
            "Warning: Skipping mu_L/nu_L plot - requires 'mu_L', 'nu_L', and 'trial_id' columns"
        )
        return
    ...
    df_unique = (
        df_mix.groupby(["L", "trial_id"])
        .agg({"mu_L": "first", "nu_L": "first"})
        .reset_index()
    )
    ...
    # Legend below figure
    fig.legend(
        handles=[line_mu, line_nu, band_nu, ref],
        labels=[
            r"$\mu_L$ median",
            r"$\nu_L$ median",
            f"{p_low_str}–{p_high_str}% band",
            rf"$\sqrt{{(M-m)/L}}$",
        ],
        loc="lower center",
...
def main(config_path: str) -> None:
    ...
    plot_combined_cdf_comparison(
        df, config, output_dir / "spurious_and_noise_cdf_comparison.pdf"
    )
    plot_cdf_and_rho_min_combined(
        df, config, output_dir / "cdf_and_rho_min_combined.pdf"
    )
